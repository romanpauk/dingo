...
TEST(object_store_test,
  ...
  {
    detail::object_store<arena<>> store(backing);
    store.construct<runtime_arena_tracker>(&events, 1);
    store.construct<runtime_arena_tracker>(&events, 2);
  }
...
TEST(object_store_test,
  ...
  {
    detail::object_store<arena<>> store(scratch);
    store.construct<runtime_arena_tracker>(&events, 1);
...
TEST(object_store_test, object_store_destroy_checkpoint_does_not_rewind_arena) {
  ...
  int *observed = std::addressof(store.construct<int>(42));
  store.construct<object_store_destroy_observer>(&events, observed);

  store.destroy(checkpoint);

  ASSERT_EQ(events, (std::vector<int>{42}));
  EXPECT_NE(std::addressof(store.construct<int>(7)), observed);
}
...
TEST(object_store_test,
  ...
  for (int value = 1; value != 40; ++value) {
    store.construct<runtime_arena_tracker>(&events, value);
  }
...
TEST(object_store_test,
  ...
  auto checkpoint = store.mark();
  auto &first = store.construct<object_store_trivial_value>(42);
