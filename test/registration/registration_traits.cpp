...
TEST(type_registration_test, recursive_leaf_and_rebind_traits) {
  ...
  using plain_pointer_requests =
      detail::binding_request_types<A,
                                    typename plain_pointer_model::storage_type>;
  static_assert(std::is_same_v<typename plain_pointer_requests::value_types,
                               type_list<runtime_type>>);
  static_assert(std::is_same_v<typename plain_pointer_requests::pointer_types,
                               type_list<runtime_type *>>);
  ...
  using optional_pointer_requests = detail::binding_request_types<
      A, typename optional_pointer_model::storage_type>;
  static_assert(std::is_same_v<typename optional_pointer_requests::value_types,
                               type_list<exact_lookup<std::optional<A>>>>);
  static_assert(
      std::is_same_v<typename optional_pointer_requests::lvalue_reference_types,
                     type_list<exact_lookup<std::optional<A>> &>>);
  static_assert(
      std::is_same_v<typename optional_pointer_requests::pointer_types,
                     type_list<exact_lookup<std::optional<A>> *>>);

  using external_move_only_pointer_model = detail::binding_model<
      type_registration<scope<external>, storage<move_only *>>>;
  using external_requests = detail::binding_request_types<
      move_only, typename external_move_only_pointer_model::storage_type>;
  static_assert(
      std::is_same_v<typename external_requests::value_types, type_list<>>);
  static_assert(std::is_same_v<typename external_requests::pointer_types,
                               type_list<runtime_type *>>);

  using shared_move_only_model = detail::binding_model<
      type_registration<scope<shared>, storage<move_only>>>;
  using shared_requests = detail::binding_request_types<
      move_only, typename shared_move_only_model::storage_type>;
  static_assert(
      std::is_same_v<typename shared_requests::value_types, type_list<>>);
  static_assert(std::is_same_v<typename shared_requests::lvalue_reference_types,
                               type_list<runtime_type &>>);

  using unique_move_only_model = detail::binding_model<
      type_registration<scope<unique>, storage<move_only>>>;
  using unique_requests = detail::binding_request_types<
      move_only, typename unique_move_only_model::storage_type>;
  static_assert(
      std::is_same_v<typename unique_requests::value_types,
  ...
  using compatible_unique_handle_requests = detail::binding_request_types<
      I, typename compatible_unique_handle_model::storage_type>;
  static_assert(type_list_contains_v<
                std::unique_ptr<runtime_type>,
                typename compatible_unique_handle_requests::value_types>);

  using incompatible_unique_handle_model =
      detail::binding_model<type_registration<
          scope<unique>, storage<std::unique_ptr<A, incompatible_deleter>>,
          interfaces<I>>>;
  using incompatible_unique_handle_requests = detail::binding_request_types<
      I, typename incompatible_unique_handle_model::storage_type>;
  static_assert(!type_list_contains_v<
                std::unique_ptr<runtime_type>,
                typename incompatible_unique_handle_requests::value_types>);
  static_assert(
      !type_list_contains_v<std::unique_ptr<runtime_type> &&,
                            typename incompatible_unique_handle_requests::
                                rvalue_reference_types>);

  using fresh_requests =
      detail::binding_request_types<move_only, fresh_value_storage<move_only>>;
  static_assert(std::is_same_v<typename fresh_requests::value_types,
                               type_list<runtime_type>>);
  using immovable_fresh_requests =
      detail::binding_request_types<immovable, fresh_value_storage<immovable>>;
  static_assert(std::is_same_v<typename immovable_fresh_requests::value_types,
                               type_list<>>);
  ...
  using shared_variant_requests = detail::binding_request_types<
      std::shared_ptr<move_only>, typename shared_variant_model::storage_type>;
  static_assert(
      std::is_same_v<typename shared_variant_requests::value_types,
                     type_list<exact_lookup<std::shared_ptr<move_only>>>>);
  ...
  using shared_move_only_variant_requests = detail::binding_request_types<
      std::unique_ptr<move_only>,
      typename shared_move_only_variant_model::storage_type>;
  static_assert(
      std::is_same_v<typename shared_move_only_variant_requests::value_types,
                     type_list<>>);
  ...
  using unique_variant_requests = detail::binding_request_types<
      std::unique_ptr<move_only>, typename unique_variant_model::storage_type>;
  static_assert(
      std::is_same_v<typename unique_variant_requests::value_types,
                     type_list<exact_lookup<std::unique_ptr<move_only>>>>);
  ...
  using nested_requests = detail::binding_request_types<
      move_only,
      typename external_nested_move_only_pointer_model::storage_type>;
  static_assert(
      std::is_same_v<typename nested_requests::value_types, type_list<>>);
  static_assert(
      std::is_same_v<typename nested_requests::pointer_types,
  ...
  using const_array_pointer_requests = detail::binding_request_types<
      A, typename const_array_pointer_model::storage_type>;
  static_assert(
      std::is_same_v<typename const_array_pointer_requests::value_types,
                     type_list<exact_lookup<std::array<A, 2>>>>);
  static_assert(std::is_same_v<
                typename const_array_pointer_requests::lvalue_reference_types,
                type_list<exact_lookup<const std::array<A, 2>> &>>);
  static_assert(
      std::is_same_v<typename const_array_pointer_requests::pointer_types,
                     type_list<exact_lookup<const std::array<A, 2>> *>>);

  using unique_optional_pointer_model = detail::binding_model<
      type_registration<scope<unique>, storage<optional_pointer>>>;
  using unique_optional_pointer_requests = detail::binding_request_types<
      A, typename unique_optional_pointer_model::storage_type>;
  static_assert(std::is_same_v<
                typename unique_optional_pointer_requests::value_types,
                type_list<exact_lookup<std::unique_ptr<std::optional<A>>>,
                          exact_lookup<std::shared_ptr<std::optional<A>>>>>);
  static_assert(
      std::is_same_v<
          typename unique_optional_pointer_requests::rvalue_reference_types,
          type_list<exact_lookup<std::unique_ptr<std::optional<A>>> &&,
                    exact_lookup<std::shared_ptr<std::optional<A>>> &&>>);
  static_assert(
      std::is_same_v<typename unique_optional_pointer_requests::pointer_types,
                     type_list<exact_lookup<std::optional<A>> *>>);
}
