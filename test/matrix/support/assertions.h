...
[[noreturn]] inline void assertion_failed(
    ...
    throw assertion_error(
        std::string(file) + ":" + std::to_string(line) + ": assertion failed: " +
        expression);
}

class fail_expression {
  public:
    fail_expression(const char* file, int line)
        : file_{file}
        , line_{line} {}
...
inline void assert_string_equal(
    const char* left,
    const char* right,
    const char* left_expression,
    const char* right_expression,
...
#define ASSERT_EQ(left, right)                                                 \
    do {                                                                       \
        ::dingo::matrix::assert_equal(                                         \
            (left), (right), #left, #right, __FILE__, __LINE__);               \
    } while (false)

#define ASSERT_NE(left, right)                                                 \
    do {                                                                       \
        ::dingo::matrix::assert_not_equal(                                     \
            (left), (right), #left, #right, __FILE__, __LINE__);               \
    } while (false)

#define ASSERT_GT(left, right)                                                 \
    do {                                                                       \
        ::dingo::matrix::assert_greater(                                       \
            (left), (right), #left, #right, __FILE__, __LINE__);               \
    } while (false)

#define ASSERT_GE(left, right)                                                 \
    do {                                                                       \
        ::dingo::matrix::assert_greater_equal(                                 \
            (left), (right), #left, #right, __FILE__, __LINE__);               \
    } while (false)

#define ASSERT_LE(left, right)                                                 \
    do {                                                                       \
        ::dingo::matrix::assert_less_equal(                                    \
            (left), (right), #left, #right, __FILE__, __LINE__);               \
    } while (false)

#define ASSERT_STREQ(left, right)                                              \
    do {                                                                       \
        ::dingo::matrix::assert_string_equal(                                  \
            (left), (right), #left, #right, __FILE__, __LINE__);               \
    } while (false)
...
#define ASSERT_THROW(statement, exception_type)                                \
    do {                                                                       \
        ...
        if (!dingo_matrix_assertion_caught) {                                  \
            ::dingo::matrix::assertion_failed(                                 \
                "expected " #exception_type " from " #statement,              \
                __FILE__,                                                      \
                __LINE__);                                                     \
        }                                                                      \
    } while (false)
