...
TEST(index_test, same_interface_can_use_multiple_key_types) {
  ...
  ASSERT_EQ(container.template resolve<animal &>(std::size_t(2)).id(), 2);
...
TEST(index_test, unqualified_key_value_broadcasts_to_matching_interfaces) {
  struct processor {
    virtual ~processor() = default;
    virtual int processor_id() const = 0;
  };
  ...
  struct traits : dynamic_container_traits {
    using lookup_definition_type =
        lookups<associative<int, processor>, associative<int, animal>>;
  };
  ...
  ASSERT_EQ(container.template resolve<processor &>(7).processor_id(), 9);
  ASSERT_EQ(container.template resolve<animal &>(7).animal_id(), 4);
}

TEST(index_test, qualified_key_value_disambiguates_matching_interfaces) {
  struct processor {
    virtual ~processor() = default;
    virtual int processor_id() const = 0;
  };
  ...
  container.template register_type<scope<shared>, storage<processor_animal>,
                                   interfaces<processor, animal>>(
      key_value<int, processor>{7}, key_value<int, animal>{9});

  ASSERT_EQ(container.template resolve<processor &>(7).processor_id(), 9);
  ASSERT_EQ(container.template resolve<animal &>(9).animal_id(), 4);
  EXPECT_THROW((container.template resolve<processor &>(9)),
               type_not_found_exception);
  EXPECT_THROW((container.template resolve<animal &>(7)),
               type_not_found_exception);
}

TEST(index_test, qualified_key_value_overrides_unqualified_broadcast) {
  struct processor {
    virtual ~processor() = default;
    virtual int processor_id() const = 0;
  };
  ...
  container.template register_type<scope<shared>, storage<processor_animal>,
                                   interfaces<processor, animal>>(
      key_value<int>{7}, key_value<int, animal>{9});
...
TEST(index_test, key_type_resolves_fixed_indexed_associative_binding) {
  using source = bindings<
  ...
  auto &selected = container.template resolve<
