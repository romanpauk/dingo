...
struct dynamic_container_with_index : dynamic_container_traits {
    using index_definition_type =
        indexes<index<Interface, IndexKey, IndexType>>;
};
...
TEST(index_test, index_tag) {
    ...
    static_assert(
        std::is_same_v<typename index_tag<std::size_t, definitions>::type,
                       char>);
}
...
TEST(index_test, index_tag_selects_exact_interface_and_key) {
    ...
    using definitions = indexes<
        index<indexed_interface, std::size_t, index_type::array<3>>,
...
TEST(index_test, same_key_type_can_use_different_array_sizes_per_interface) {
    ...
    container.template register_indexed_type<
        ...
        std::size_t(2));
    container.template register_indexed_type<
        scope<shared>, storage<animal_impl>, interfaces<animal>>(
        std::size_t(15));
...
TEST(index_test, constructor_injects_dependencies_from_two_array_indexes) {
    ...
    container<traits> container;
    container.template register_indexed_type<
        scope<shared>, storage<file_source>, interfaces<source>>(
        std::size_t{0});
    container.template register_indexed_type<
...
TEST(index_test, custom_backend_can_use_container_allocator) {
    ...
    struct traits : dynamic_container_traits {
        using index_definition_type = indexes<
            index<animal, std::string, index_type::test_allocator_vector>>;
    };
...
TEST(index_test,
    ...
    struct consumer {
        explicit consumer(
            indexed<processor&, external_key_string<std::string, "json">>
                selected)
    ...
    struct traits : dynamic_container_traits {
        using index_definition_type = indexes<
            index<processor, key<std::string>, index_type::unordered_map>>;
    };
...
TEST(index_test, constructor_injects_fixed_indexed_map_binding) {
    struct traits : dynamic_container_traits {
        using index_definition_type = indexes<
            index<fixed_injection_processor, std::size_t, index_type::map>>;
    };
...
TEST(index_test, constructor_injects_fixed_indexed_enum_key) {
    struct traits : dynamic_container_traits {
        using index_definition_type =
            indexes<index<fixed_injection_processor,
                          fixed_injection_processor_id, index_type::map>>;
    };
...
TEST(index_test, selected_type_selector_registers_and_injects_keyed_binding) {
    ...
    container.template register_type<scope<shared>,
                                     storage<selected_type_consumer>>();
...
TEST(index_test, annotated_interfaces_keep_independent_indexes) {
    ...
    struct traits : dynamic_container_traits {
        using index_definition_type =
            indexes<index<annotated<fixed_injection_processor,
                                    annotated_index_primary_tag>,
                          std::size_t, index_type::array<2>>,
                    index<annotated<fixed_injection_processor,
