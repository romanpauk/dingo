...
template <class T> struct decoration_traits {
  ...
  template <class U> using rebind_t = U;
};
...
template <class T, class U> struct lookup_base<exact_lookup<T>, U, void> {
...
template <class T, class U>
struct resolved_base<exact_lookup<T>, U,
                     std::enable_if_t<!std::is_same_v<
                         typename leaf_type<T>::type, runtime_type>>> {
  using type = T;
};
...
private:
  using outer = detail::outer_traits<T>;
  using rebound =
      typename detail::rebind_leaf_base<typename outer::type, U>::type;
...
private:
  using outer = detail::outer_traits<T>;
  using rebound =
      typename detail::lookup_base<typename outer::type, runtime_type>::type;

public:
  using type = typename outer::template rebind_t<rebound>;
};

template <typename T> using lookup_type_t = typename lookup_type<T>::type;
