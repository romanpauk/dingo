...
template <typename T> std::false_type is_complete_impl(...);

template <typename T> struct is_complete : decltype(is_complete_impl<T>(0)) {};
...
struct is_complete : detail::is_complete<T> {};
} // namespace dingo
