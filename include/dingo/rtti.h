...
class dynamic_rtti {
  public:
    class type_index {
      public:
        type_index(std::type_index value, bool smart_ptr)
            : value_(value), is_smart_ptr_(smart_ptr) {}
    ...
    template <typename T> static type_index get_type_index() {
        return {
            ...
            type_traits<std::remove_pointer_t<std::decay_t<T>>>::is_smart_ptr};
    }
};
...
template <typename T>
const size_t static_type_index_cache<T>::value =
    static_type_index_base<void>::generate_id();
  ...
  public:
    ...
    template <typename T> static type_index get_type_index() {
        return static_type_index_cache<T>::value |
               type_traits<
                   std::remove_pointer_t<std::decay_t<T>>>::is_smart_ptr;
    }
};
} // namespace dingo